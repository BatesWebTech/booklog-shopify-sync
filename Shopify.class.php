...
class Shopify {
	...
	function getProductCount() {
		$resp = $this->call('get','products/count.json');
		$resp = json_decode($resp);
		return $resp->count;
	}

	function getProducts($fields=array()){
		$products = $this->call('get','products.json',$fields);
	...
	function getProduct($id,$fields=array()){
		$result = $this->call('get',"products/{$id}.json",$fields);
	...
	function updateItem($type,$id,$updatefields){
		...
		$result = $this->call('put',"{$type}s/{$id}.json",$data);
